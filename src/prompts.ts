...
export class Prompts {
  ...
  summarizeFileDiff = `## Gitlab MR Title
...
and the goal of this MR. This summary will be used as a prompt while reviewing each
file and must be very clear for the AI bot to understand. The summary should not
exceed 250 words.
`

  reviewFileDiff = `## Gitlab MR Title
...
goal of this MR. The line number annotation on each line in the new
...
- IMPORTANT: Line number ranges for each review section must be within the
  ...
  your comment to the code changes in the Gitlab pull request.
...
`
  ...
  comment = `A comment was made on a Gitlab MR review for a
